...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#1c1917] text-stone-200 selection:bg-red-900 selection:text-white overflow-x-hidden font-serif pb-10">
      ...
      {/* Header */}
      <header className="relative z-10 pt-6 pb-2 px-4 text-center border-b border-stone-800/50 bg-[#1c1917]/80 backdrop-blur-md sticky top-0 md:static">
        <div className="flex items-center justify-center gap-4">
           ...
           <h1 className="text-2xl md:text-4xl font-calligraphy text-[#e7e5e4] drop-shadow-md">
             平安京 · 数学绘卷
           </h1>
        </div>
      </header>
      ...
      {/* Main Content */}
      <main className="relative z-10 max-w-7xl mx-auto px-4 md:px-6 pt-6 pb-12">
        
        {/* New Navigation: Soul (Mitama) Inventory Style */}
        <div className="mb-6 md:mb-8 p-3 md:p-4 bg-stone-900/30 border border-stone-800 rounded-xl relative overflow-hidden backdrop-blur-sm">
           <div className="absolute top-0 left-0 text-[10px] bg-stone-800 px-2 py-0.5 text-stone-500 uppercase tracking-widest rounded-br z-10">御魂选择 · Soul Select</div>
           
           {/* Horizontal Scroll on Mobile, Wrap on Desktop */}
           <div className="flex overflow-x-auto pb-2 gap-4 pt-4 px-1 hide-scrollbar snap-x snap-mandatory md:flex-wrap md:justify-center md:overflow-visible">
              {Object.values(SubjectType).map((sub, index) => {
                const chap = CHAPTERS.find(c => c.subject === sub);
                if (!chap) return null;
                
                return (
                  <div key={sub} className="snap-center">
                    <SubjectCard 
                      index={index}
                      subject={sub}
                      title={chap.title}
           ...
           <div className="md:hidden text-center text-[10px] text-stone-600 mt-1 animate-pulse">
              ← 滑动查看更多御魂 →
           </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-auto">
          ...
          {/* On Mobile (Order 1): Shows at top to give context of what section we are in */}
          <div className="lg:col-span-3 flex flex-col gap-3 order-1">
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-3">
                           <div className={`
                                w-7 h-7 rounded flex items-center justify-center shadow-inner shrink-0
                        ...
                        <div className="flex items-center gap-2 overflow-hidden">
                           <span className={`
                                text-sm font-serif text-right truncate
                                ${isActive ? 'text-stone-100 font-bold' : 'text-stone-400 group-hover:text-stone-300'}
                           `}>
                                {section.title.split('=')[0].trim()}
                           </span>
                           {isActive && <div className="w-1.5 h-1.5 rounded-full bg-amber-500 animate-pulse shrink-0" />}
                        </div>
                      </button>
                    );
                  })}
                  
                  {/* Fills empty slots to make it look like a full panel if few sections - Hidden on Mobile to save space */}
          ...
          {/* Main Content */}
          <div className="lg:col-span-9 flex flex-col h-full order-2">
            <ScrollContainer title={activeSection.title}>
              <div className={`transition-opacity duration-300 ${isTransitioning ? 'opacity-0' : 'opacity-100'}`}>
                
                {/* Flex-col-reverse on mobile puts image on top (visually), text below */}
                <div className="flex flex-col-reverse xl:flex-row gap-6 md:gap-8 mb-8">
                    
                    <div className="flex-1 space-y-6">
                       <div className="text-base md:text-lg text-stone-800 leading-loose font-serif">
                          <MathParser text={activeSection.content} />
                       </div>
                       
                       {activeSection.formula && (
                        <div className="bg-stone-800 text-amber-50 rounded-lg p-4 md:p-5 shadow-xl border-l-4 border-amber-600 relative overflow-x-auto">
                    ...
                    <div className="w-full xl:w-64 flex-shrink-0 flex justify-center xl:block">
                      <div className="bg-stone-200 p-2 rounded shadow-inner border border-stone-300 -rotate-1 hover:rotate-0 transition-transform duration-500 max-w-[200px] xl:max-w-none w-full">
                   ...
                   <div className="bg-[#1f1f1f] rounded border border-stone-600 overflow-hidden font-mono text-sm shadow-md mt-4">
                     <div className="bg-[#151515] px-4 py-2 border-b border-stone-600 flex justify-between items-center">
                       <span className="text-stone-400 font-bold flex items-center gap-2 text-xs tracking-wider">
                         <Calculator className="w-3 h-3" /> COMBAT_SIMULATION
                       </span>
                       <span className="text-[10px] text-stone-600 hidden sm:inline">LOG_ID: {Math.floor(Math.random() * 99999)}</span>
                     </div>
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-2">
                        {/* Correct Scenario */}
                        <div className="p-4 bg-green-900/10 border-b md:border-b-0 md:border-r border-stone-700/50">
            ...
            <div className="flex justify-between items-center mt-4 px-2 pb-6 md:pb-0">
               <button 
                 onClick={prevSection}
                 disabled={safeSectionIndex === 0}
                 className={`flex items-center gap-1 text-sm font-bold uppercase tracking-widest transition-all p-2
               ...
               <div className="flex gap-1">
                  {currentChapter.sections.map((_, i) => (
                    <div 
                      key={i} 
                      className={`h-1.5 rounded-full transition-all duration-300 ${i === safeSectionIndex ? 'w-6 md:w-8 bg-amber-600' : 'w-1.5 md:w-2 bg-stone-700'}`}
                    />
                  ))}
               </div>
